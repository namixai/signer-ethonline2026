...
| **Reproducible build + attestation** | PCR0 rebuilds from a clean clone; a live NSM COSE document is served | `signer-demo.usenami.io/attestation` — **the demo box**. We run more than one enclave; ask this one and you are asking the demo lane, not mainnet. Registration of a measurement is checked on chain (`isPCR0Active`, Base), never by the `registered_onchain` field in that response — that field is an operator setting, not a chain read. |
