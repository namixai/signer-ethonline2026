...
🔴 **`spendUnknown: true` ставится только тогда, когда это правда неизвестно.** Признак
структурный: ушёл ли в провод заголовок платежа — `X-PAYMENT` либо `PAYMENT-SIGNATURE`,
которым платит повтор по x402 v2. Смотреть надо на оба: проверка одного пропустила бы
половину боевых платежей и молча вернула бы `spendUnknown: false` там, где деньги как
раз могли уйти. Если ушёл, бросок мог означать и
что платёж дошёл, а ответ потерялся, — записать такое как «трат не было» значило бы
угадать, а угадывать про деньги нельзя.

Если не ушёл, гадать не о чем, и флаг стоит `false`. Отказ спенд-контроля — как раз
этот случай: потолок режет **до того**, как payload собран, подписи не существует.
Раньше он приходил как `paid_request_failed` + `spendUnknown: true`, то есть отправлял
читателя искать в обозревателе блоков платёж, которого никогда не было, — и приходил на
каждом прогоне при кривом потолке, обесценивая флаг ровно там, где он нужен. Теперь у
него своё имя: `payment_over_cap`, и смотреть надо не в цепь, а в `X402_MAX_PER_PAYMENT`.
...
`0xE1D1D3526A6FAa37eb36bD10B933C1b77f4561a4`. Пять дней никто не заметил, **потому что
этот путь никто не выполнял** — ровно тот же класс, что `wrapFetchWithPayment` с
viem-аккаунтом вместо клиента. Подсадка старого адреса роняет живой тест, что и
доказывает: тест сверяется с миром, а не с собой.

🔴 **Здесь же было написано «нет контракта вообще» — и это неправда.** Замер 12.09
через `eth_getCode`: по этому адресу **3569 байт кода в Base (8453)** и ноль в World
Chain (480). Байткод в Base совпадает с запечённым AgentBook в 480
(`0xA23aB2712eA7BBa896930544C7d6636a96b944dA`) **побайтово** — те же 3569 байт, тот же
sha256. Наш собственный `AGENT_BOOK_DEPLOYMENTS` числит `0xE1D1…` как деплой в Base.

То есть долг был не в мёртвом адресе, а в адресе без цепи: адрес живой, читали его в
другой цепи и получали пусто. Это ровно та ловушка, о которой предупреждает
`world.js` — объект цепи обязан совпадать с RPC. Неверный диагноз своего же бага
опаснее самого бага: он учит искать не там.
